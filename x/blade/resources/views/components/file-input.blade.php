@props(["title"=>"", "multiple"=>false, "name"=>"", "id"=>""])
<div class="mt-3">
    @if ($title!="")
    <x-input-label for="{{$id}}">{{$title}} :</x-input-label>
        
    @endif
    <div class="flex items-center mt-2 relative h-10">
        <input type="file" id="{{$id}}" name="{{$name}}" {{$multiple ? 'multiple' : ''}} required class="w-full opacity-0 absolute" accept="image/*">
        <span id="file-name" class=" flex-1 p-2 line-clamp-1 space-y-1 bg-white h-full text-gray-700">Pilih File</span>

        <label for="photos" class="flex items-center justify-center px-4 py-2 bg-gradient-to-r from-indigo-400 to-cyan-400 text-white  cursor-pointer hover:bg-red-500">
            <span class="mr-2">
